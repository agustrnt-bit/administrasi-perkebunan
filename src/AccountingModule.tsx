...
export default function AccountingModule({ data, flash, showError }: Props) {
  ...
  const accountingModeLabel = mode === 'journal' ? 'Jurnal Umum' : mode === 'manual' ? 'Jurnal Manual' : mode === 'coa' ? 'Chart of Accounts' : mode === 'system' ? 'Akun Penting' : mode === 'opening' ? 'Saldo Awal' : mode === 'periods' ? 'Periode Akuntansi' : mode === 'ledger' ? 'Buku Besar' : mode === 'trial' ? 'Neraca Saldo' : 'Akuntansi';
  ...
  return <div className="stack">
    {mode === 'hub' ? <>
      ...
      <div className="master-hub"><section className="master-hub-group"><div className="master-hub-group-head"><div><strong>Akuntansi</strong><span>Setup akun, jurnal, buku besar dan kontrol saldo.</span></div></div><div className="master-hub-grid">
        <button type="button" className="master-hub-card" onClick={() => changeMode('journal')}><span className="master-hub-card-icon"><BookOpen size={24} /></span><span className="master-hub-card-copy"><strong>Jurnal Umum</strong><small>Lihat jurnal otomatis dan jurnal penyesuaian pada satu daftar.</small></span><ChevronRight size={18} /></button>
        <button type="button" className="master-hub-card" onClick={() => changeMode('manual')}><span className="master-hub-card-icon"><FilePenLine size={24} /></span><span className="master-hub-card-copy"><strong>Jurnal Manual</strong><small>Input jurnal penyesuaian yang tidak berasal dari modul operasional.</small></span><ChevronRight size={18} /></button>
        ...
        <button type="button" className="master-hub-card" onClick={() => changeMode('ledger')}><span className="master-hub-card-icon"><Landmark size={24} /></span><span className="master-hub-card-copy"><strong>Buku Besar</strong><small>Mutasi Debit/Kredit dan saldo berjalan per akun Level 4.</small></span><ChevronRight size={18} /></button>
        <button type="button" className="master-hub-card" onClick={() => changeMode('trial')}><span className="master-hub-card-icon"><Scale size={24} /></span><span className="master-hub-card-copy"><strong>Neraca Saldo</strong><small>Kontrol saldo Debit/Kredit seluruh akun hingga akhir periode.</small></span><ChevronRight size={18} /></button>
      </div></section></div>
