...
export default function AccountingFoundation({ data, accounts, systemAccountIds = {}, reloadAccounts, flash, showError, section: fixedSection }: Props) {
  ...
  return <div className="stack">
    ...
    </section><section className="panel"><div className="purchase-section-head"><div><strong>Detail Aset Tetap</strong><span>Ditarik langsung dari Master Data → Daftar Aset Tetap. Akumulasi mengikuti perhitungan otomatis sampai cut-off atau Override Manual khusus migrasi.</span></div></div>{openingFixedAssets.length === 0 ? <div className="empty"><span>Belum ada aset yang masuk cut-off Saldo Awal.</span></div> : <div className="table-wrap"><table><thead><tr><th>Kode / Aset</th><th>Kelompok</th><th>Cost Center</th><th className="right">Harga Perolehan</th><th className="right">Akumulasi</th><th className="right">Nilai Buku Awal</th></tr></thead><tbody>{openingFixedAssets.map(asset => { const group = fixedAssetGroupMap.get(asset.groupId); const kebun = data.kebun.find(item => item.id === asset.kebunId); return <tr key={asset.id}><td><strong>{asset.code}</strong> · {asset.name}</td><td>{group?.name || '-'}</td><td>{asset.kebunId ? kebun?.name || '-' : 'Pusat / Umum'}</td><td className="right">{idr.format(asset.acquisitionCost)}</td><td className="right">{idr.format(asset.openingAccumulatedDepreciation)}</td><td className="right"><strong>{idr.format(Math.max(0, asset.acquisitionCost - asset.openingAccumulatedDepreciation))}</strong></td></tr>; })}</tbody></table></div>}</section><section className="panel"><div className="purchase-section-head"><div><strong>Detail Subledger</strong><span>Detail ini otomatis menentukan akun GL dan menjadi kontrol rekonsiliasi saldo awal.</span></div>{opening?.status !== 'POSTED' && <button className="secondary small-btn" onClick={() => setSubledgers(rows => [...rows, blankSub()])}><Plus size={14} /> Detail</button>}</div><div className="subledger-lines">{subledgers.map((row, index) => { const options = entityOptions(row.kind); const item = inventoryItems.find(candidate => candidate.id === row.entityId); return <div className="subledger-line" key={index}><select disabled={opening?.status === 'POSTED'} value={row.kind} onChange={e => setSub(index, { kind: e.target.value as SubKind, entityId: '', amount: '', quantity: '', unitCost: '' })}><option value="CASH_BANK">Kas / Bank</option><option value="PKS">Piutang PKS</option><option value="EMPLOYEE">Piutang Karyawan</option><option value="SUPPLIER">Hutang Supplier</option><option value="INVENTORY">Persediaan</option></select><select disabled={opening?.status === 'POSTED'} value={row.entityId} onChange={e => setSub(index, { entityId: e.target.value })}><option value="">Pilih detail</option>{options.map(option => <option key={option.id} value={option.id}>{option.label}</option>)}</select><div className="subledger-account"><span>Akun</span><strong>{subAccountName(row)}</strong></div>{row.kind === 'INVENTORY' ? <><input disabled={opening?.status === 'POSTED'} inputMode="decimal" placeholder={`Qty ${unitMap.get(item?.unitId || '')?.code || ''}`} value={row.quantity} onChange={e => setSub(index, { quantity: e.target.value.replace(',', '.').replace(/[^0-9.]/g, '') })} /><input disabled={opening?.status === 'POSTED'} inputMode="numeric" placeholder="Average awal" value={formatMoneyInput(row.unitCost)} onChange={e => setSub(index, { unitCost: e.target.value.replace(/[^0-9]/g, '') })} /><strong className="subledger-total">{idr.format(subAmount(row))}</strong></> : <input disabled={opening?.status === 'POSTED'} inputMode="numeric" placeholder="Nominal" value={formatMoneyInput(row.amount)} onChange={e => setSub(index, { amount: e.target.value.replace(/[^0-9]/g, '') })} />}<input disabled={opening?.status === 'POSTED'} placeholder="Referensi / invoice / DO" value={row.reference} onChange={e => setSub(index, { reference: e.target.value })} />{opening?.status !== 'POSTED' && subledgers.length > 1 && <button className="icon-btn danger" onClick={() => setSubledgers(rows => rows.filter((_, i) => i !== index))}><Trash2 size={14} /></button>}</div>; })}</div></section>{opening?.status !== 'POSTED' && <section className="panel opening-actions"><div><strong>Posting hanya jika Debit = Kredit</strong><span>Posting akan mengunci Saldo Awal Kas/Bank, Persediaan, serta harga perolehan dan akumulasi awal dari Daftar Aset Tetap.</span></div><div className="action-group"><button className="secondary" disabled={saving || !settings?.setupComplete} onClick={saveOpening}><Save size={16} /> Simpan Draft</button><button className="primary" disabled={saving || !settings?.setupComplete || previewDebit <= 0 || previewDebit !== previewCredit} onClick={postOpening}>Posting Saldo Awal</button></div></section>}</>}
  </div>;
}
