...
export default function InventoryUsage({ workspaceId, role, kebun, flash, showError }: Props) {
  ...
  return <div className="stack">
    ...
    {mode === 'stocktake' && <StocktakePanel role={role} canEdit={canEdit} warehouses={warehouses} items={items} flash={flash} showError={showError} reload={load} />}
  </div>;
}
...
function TransferPanel({ canEdit, warehouses, balances, items, units, flash, showError, reload }: { canEdit: boolean; warehouses: Warehouse[]; balances: WarehouseBalance[]; items: InventoryItem[]; units: InventoryUnit[]; flash: (text: string) => void; showError: (text: string) => void; reload: () => Promise<void> }) {
  const active = warehouses.filter(item => item.active); const defaultId = active.find(item => item.isDefault)?.id || active[0]?.id || ''; const [transfers, setTransfers] = useState<Transfer[]>([]); const [date, setDate] = useState(today()); const [sourceId, setSourceId] = useState(defaultId); const [destinationId, setDestinationId] = useState(''); const [reference, setReference] = useState(''); const [description, setDescription] = useState('Mutasi Antar Gudang'); const [lines, setLines] = useState<Array<{ itemId: string; quantity: string }>>([{ itemId: '', quantity: '' }]); const [saving, setSaving] = useState(false);
  ...
  const save = async (event: React.FormEvent) => { event.preventDefault(); if (!date || !sourceId || !destinationId || sourceId === destinationId || lines.some(line => !line.itemId || Number(line.quantity) <= 0)) return showError('Tanggal, gudang asal/tujuan yang berbeda, barang, dan qty wajib diisi.'); try { setSaving(true); await api.post('/api/inventory-transfers', { date, sourceWarehouseId: sourceId, destinationWarehouseId: destinationId, reference, description, lines: lines.map(line => ({ itemId: line.itemId, quantity: Number(line.quantity) })) }); setLines([{ itemId: '', quantity: '' }]); setReference(''); await Promise.all([reload(), loadTransfers()]); flash('Mutasi Antar Gudang berhasil diposting.'); } catch (err) { showError(apiError(err, 'Mutasi Antar Gudang gagal disimpan.')); } finally { setSaving(false); } };
...
function StocktakePanel({ role, canEdit, warehouses, items, flash, showError, reload }: { role: Role; canEdit: boolean; warehouses: Warehouse[]; items: InventoryItem[]; flash: (text: string) => void; showError: (text: string) => void; reload: () => Promise<void> }) {
  ...
  const open = (item: Stocktake) => { setSelectedId(item.id); setPic(item.pic || ''); setNote(item.note || ''); const next: Record<string, { physicalQuantity: string; valuationCost: string; note: string }> = {}; item.lines.forEach(line => { next[line.itemId] = { physicalQuantity: line.physicalQuantity == null ? '' : String(line.physicalQuantity), valuationCost: line.unitCost > 0 ? String(line.unitCost) : '', note: line.note || '' }; }); setDraft(next); };
  ...
  const saveCount = async (submitReview: boolean) => { if (!selected) return; try { setSaving(true); await api.put(`/api/inventory-stocktakes/${selected.id}`, { date: selected.date, pic, note, submitReview, lines: selected.lines.map(line => ({ itemId: line.itemId, physicalQuantity: draft[line.itemId]?.physicalQuantity ?? '', valuationCost: Number(draft[line.itemId]?.valuationCost || 0), note: draft[line.itemId]?.note || '' })) }); await loadStocktakes(); await reload(); flash(submitReview ? 'Stok Opname masuk tahap Review.' : 'Hitungan fisik Stok Opname disimpan.'); } catch (err) { showError(apiError(err, 'Stok Opname gagal disimpan.')); } finally { setSaving(false); } };
